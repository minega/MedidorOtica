//
//  CameraView_teste.swift
//  MedidorOticaApp
//
//  teste de modificacao Tela de captura de imagem para medições de ótica.
...
struct CameraView: View {
    ...
    func configureCameraProcessing() {
        print("Configurando processamento de frames da câmera...")
        
        // Configura o delegate para processar cada frame
        cameraManager.outputDelegate = { [weak self] frame in
            guard let self = self else { return }
            
            // Envia o frame para processamento no VerificationManager
            // O processamento é feito em background para não bloquear a thread principal
            self.verificationManager.processARFrame(frame)
        }
        
        // Inicia a sessão da câmera
        cameraManager.start()
        
        // Configura um timer para atualizações periódicas da interface
        // Isso garante que a interface seja atualizada mesmo se algum frame for perdido
        if verificationTimer == nil {
            verificationTimer = Timer.scheduledTimer(withTimeInterval: 0.1, repeats: true) { [weak self] _ in
                guard let self = self else { return }
                
                // Atualiza a interface com o estado atual das verificações
                // Isso garante que a UI seja atualizada mesmo sem novos frames
                self.verificationManager.updateVerificationStatus(throttled: true)
